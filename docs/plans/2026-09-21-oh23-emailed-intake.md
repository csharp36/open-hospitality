...
- Modify: `tests/test_models.py` (`test_tables_registered` set), `tests/test_l2_rls_wall.py` (the org-walled inventory literal), `tests/test_l4_org_grants.py` (head literal → `"o1a0intake"`), `tests/test_migration_on_populated_data.py` (`_L1_ORG_INDEPENDENT` does NOT gain `"property_intake_address"` — that set means "no `org_id` column at all", and the address table carries a NOT NULL `org_id`; leave a comment saying so), `tests/test_l1_org_wall_migration.py` (the `ix_<table>_org_id` index set gains `ix_email_intake_event_org_id`; `property_intake_address` declares no such index, because it carries `org_id` without the `OrgScoped` mixin — say so in a comment)

- [x] **Step 1: failing tests.** Add the two table names to `test_tables_registered`; add `email_intake_event` to the RLS inventory; change the head literal; do NOT add the address table to `_L1_ORG_INDEPENDENT` (it is tenant-owned there — org-keyed, just unwalled); add `ix_email_intake_event_org_id` to `test_l1_org_wall_migration`'s index set. Run those five tests: they fail (tables missing / head mismatch).

- [x] **Step 2: models.** In `models.py`, next to `Invite`:
...
- [x] `sign.js`: `export async function sign(secret, timestamp, bodyBytes)` → hex via `crypto.subtle` HMAC-SHA256 over `timestamp + "\n" + body`; the test asserts the same vector Task 2 hard-coded (secret `"s"`, timestamp `"1700000000"`, body `"hello"`).
- [x] `worker.js`: `export default { async email(message, env, ctx) { … } }` per D-OH23.1: read `message.raw` into bytes; if over `env.MAX_BYTES` → forward to `env.FALLBACK_ADDRESS` and return; POST to `env.INTAKE_URL` with the five headers; on non-2xx or throw → `await message.forward(env.FALLBACK_ADDRESS)`. No parsing, no logging of bodies; `console.log` only the outcome JSON the app returns.
- [x] `wrangler.toml`: `name = "oh-email-intake"`, `main = "src/worker.js"`, `compatibility_date`, `[vars] INTAKE_URL`, `MAX_BYTES`; secrets `INTAKE_SECRET` and `FALLBACK_ADDRESS` set by the workflow from GitHub secrets `EMAIL_INTAKE_SECRET`, `EMAIL_INTAKE_FALLBACK`.
- [x] Workflow: `workflow_dispatch`; `npm ci`; `npx wrangler secret put` for the two secrets (piped from `${{ secrets.… }}`); `npx wrangler deploy`. Mirror `deploy-marketing.yml`'s comments about the token scope (this needs Workers Scripts: Edit and Email Routing Rules: Edit; the Pages token will not do — say so).
- [x] Runbook: enable Email Routing on `intake.<domain>`, verify the fallback destination, add the catch-all rule → the worker, create the Workers-scoped token, put `EMAIL_INTAKE_SECRET` in GitHub and in GCP Secret Manager (`deploy_app.sh` gains `USALI_EMAIL_INTAKE_SECRET` from the secret and `USALI_EMAIL_INTAKE_DOMAIN=intake.mandati.ai` in `COMMON_ENV`), dispatch the worker deploy, then send a test message from a DKIM-signing domain to a property's address and read the event on the property page.
- [x] `scripts/cloud/deploy_app.sh`: the two env entries above (secret via the same mount pattern the SMTP password uses).
- [x] Commit `feat(intake): Cloudflare email worker, deploy workflow, runbook`.

### Task 7: roadmap, full gates, PR

- [x] `.github/roadmap.yml` OH-23 → `shipped  # PR #<n>, 2026-09-…`; `docs/ROADMAP.md` Tier 0 row 4 Why column gains "Shipped: Cloudflare Email Worker → signed webhook; per-property token address; sender policy; event log on the property page (design 2026-09-21)"; §7 gains a dated deltas note. `docs/ARCHITECTURE.md`: one sentence on the intake path.
- [x] `uv run --frozen ruff check && uv run --frozen mypy --strict src && uv run --frozen pytest -q -p no:cacheprovider`; `cd frontend && npx tsc -b --force && npx oxlint && npm test`; `cd cloudflare/email-intake && npm test`; `docker build .`.
- [x] PR body: the decisions, the tenancy shape (org-independent address table, org-bound everything else), the gate inheritance, the runbook steps that remain manual, and the test list.
