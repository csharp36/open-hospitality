...
def _skytouch_fixture_records() -> list[StatisticRecord]:
    # Generated by scripts/gen_skytouch_mock_fixtures.py (invented figures in the
    # real header shape): five curated metrics x five period columns = 25
    # records, all with pms_source SKYTOUCH.
    words = [
...
def test_skytouch_day_and_mtd_facts_promote(db_session):
    ...
    facts = db_session.execute(select(UsaliStatisticFact)).scalars().all()
    by_key = {(f.metric_code, f.period, f.is_prior_year): f.value for f in facts}
    assert len(by_key) == 25  # no two periods collapsed onto one cell
