...
from usali.models import OrgSettings, Property
...
def build_two_tenant_world(
    ...
    (org 1 already exists with its `org_settings` row and the pilot alias)."""
    # ---- ORG 1 (the founding demo world) ----------------------------------
    # Its provider is delphi (the L5 per-org config); one property, one
    # employee, and an org-wide org_admin grant — the shape a real org 1 holds.
    db_session.execute(
        update(OrgSettings).where(OrgSettings.org_id == 1).values(crm_provider="delphi")
    )
