...
from usali.models import IngestBatch, PmsDailyStatisticStage
from usali.schemas import StatisticRecord
from usali.tenancy import current_org_id, is_org_instrumented


def _row_hash(rec: StatisticRecord, file_hash: str, index: int) -> str:
    # Ordinal position keeps two identical (label, period, value) rows distinct — the
    # same no-silent-drop rationale as the financial stage hash.
    key = (
        f"{file_hash}|{index}|{rec.metric_label}|{rec.period_label}|"
        f"{rec.is_prior_year}|{rec.value}|{rec.report_type}"
    )
    return hashlib.sha256(key.encode()).hexdigest()


def stage_statistics(
    session: Session, records: Sequence[StatisticRecord], *, source_file: str, file_hash: str
) -> IngestBatch:
    ...
    for index, rec in enumerate(records):
        values: dict[str, object] = dict(
            ...
            is_prior_year=rec.is_prior_year,
            value=rec.value,
            source_file=source_file,
        ...
        stmt = insert(PmsDailyStatisticStage).values(**values).on_conflict_do_nothing(
